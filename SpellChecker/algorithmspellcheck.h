...
namespace Spellcheck_name_space {
#define maxEditDis 4
#define PrefLength_def 7
#define CountTh_def 1
#define defaultInitialCapacity 16
#define defaultCompactLevel 5
#define mini(a, b, c) (min(a, min(b, c)))

    namespace {
        int32_t Locate_charachter(const char * text, char ch)
        {
    ...
    public:
        ...
        SuggestItem() { }
        SuggestItem(const Spellcheck_name_space::SuggestItem & p)
        {
            this->count = p.count;
    ...
    public:
        ...
        WordSegmentationItem(const Spellcheck_name_space::WordSegmentationItem & p)
        {
...
// #else
    ...
    class SpellChecker {
    public:
        SpellChecker(int32_t initialCapacity = defaultInitialCapacity, int32_t maxDictionaryEditDistance = maxEditDis, int32_t prefixLength = PrefLength_def, int32_t countThreshold = CountTh_def, int32_t compactLevel = defaultCompactLevel)
        {
        ...
        bool Entry_Dictonary(const char * key, int64_t count, SuggestionStage * staging = nullptr)
        {
        ...
        void Find_best_suggestion(const char * input, Filter_ verbosity, vector<std::unique_ptr<Spellcheck_name_space::SuggestItem>> & items)
        {
            this->Find_best_suggestion(input, verbosity, this->maxDictionaryEditDistance, false, items);
        }

        void Find_best_suggestion(const char * input, Filter_ verbosity, int32_t maxEditDistance, vector<std::unique_ptr<Spellcheck_name_space::SuggestItem>> & items)
        {
            this->Find_best_suggestion(input, verbosity, maxEditDistance, false, items);
        }

        void Find_best_suggestion(const char * input, Filter_ verbosity, int32_t maxEditDistance, bool includeUnknown, vector<std::unique_ptr<Spellcheck_name_space::SuggestItem>> & suggestions)
        {
            mtx.lock();
            suggestions.clear();
            edits.clear();
            ...
            // maxEditDistance used in Find_best_suggestion can't be bigger than the maxDictionaryEditDistance
            ...
            while (candidatePointer < candidatesLen)
            {
                ...
                {
                    // skip to next candidate if Filter_.All, look no further if Filter_.Top or Closest
                ...
                if (deletesFinded != deletesEnd)
                {
                    ...
                    for (int i = 0; i < dictSuggestionsLen; ++i)
                    {
                        ...
                        {
                            if (Locate_charachter(input, suggestion[0]) < 0) distance = inputLen; else distance = inputLen - 1;
                            distance = max(inputLen, suggestionLen);
                            if (distance > maxEditDistance2 || !hashset2.insert(stringHash(suggestion)).second)
                                continue;
                        }
                ...
                if ((lengthDiff < maxEditDistance) && (candidateLen <= prefixLength))
                {
                    ...
                    if (lengthDiff >= maxEditDistance2) continue;
            ...
            if (suggestionsLen > 1)
                std::sort(suggestions.begin(), suggestions.end(), [](std::unique_ptr<Spellcheck_name_space::SuggestItem> &l, std::unique_ptr<Spellcheck_name_space::SuggestItem> & r)
            {
        ...
        bool LoadDictionary(char* corpus, int termIndex, int countIndex)
        {
            ...
            while (getline(stream, line))
            {
                ...
                if (lineParts.size() >= 2)
                {
                    ...
                    Entry_Dictonary(lineParts[termIndex], count/*, &staging*/);
                }
